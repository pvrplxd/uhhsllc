document.addEventListener( "DOMContentLoaded" , function () {
  ...
  // Fix cross-browser width inconsistency in vertical-slider gallery layout.
  // FlexSlider's doMath() has a Firefox-specific override that reads the full gallery width
  // instead of the viewport width. This breaks the vertical-slider layout where .flex-viewport
  // is narrower than the gallery. Disabling the flag before WC's deferred resize corrects the width.
  jQuery( '.ast-product-gallery-layout-vertical-slider .woocommerce-product-gallery' ).on( 'wc-product-gallery-after-init', function () {
    const gallerySlider = jQuery( this ).data( 'flexslider' );
    if ( gallerySlider && gallerySlider.isFirefox ) {
      gallerySlider.isFirefox = false;
    }
  });
