(function(){
	...
	AstraProQuickView = {
		...
		_bind: function()
		{
			...
			// ----------------------------------------------------------------------------
			// TEMPORARY WORKAROUND — CartFlows Variation Swatches compatibility.
			// CartFlows binds resetSwatches() directly to .reset_variations on DOMContentLoaded,
			// so dynamically loaded Quick View content is never covered.
			// This handler replicates that reset, scoped to the Quick View modal.
			// Coupling: .cfvsw-swatches-option, .cfvsw-selected-swatch, .cfvsw-selected-label
			// TODO: Remove once CartFlows exposes a public reinit API or listens to a
			//       standard WooCommerce dynamic-content event (e.g. wc_variation_form_init).
			// ----------------------------------------------------------------------------
			jQuery( '#ast-quick-view-modal' ).on( 'click', '.reset_variations', function() {
				const $modal = jQuery( '#ast-quick-view-modal' );
				$modal.find( '.cfvsw-swatches-option' ).removeClass( 'cfvsw-selected-swatch' );
				$modal.find( '.cfvsw-selected-label' ).remove();
			} );
		...
		_open_quick_view: function( e ) {
			...
			xhrRequest.send( 'action=ast_load_product_quick_view&nonce=' + ( astra.quick_view_nonce || '' ) + '&product_id=' + product_id );
			xhrRequest.responseType = 'text';
			xhrRequest.onreadystatechange = function () {
				if ( xhrRequest.readyState == XMLHttpRequest.DONE ) {   // XMLHttpRequest.DONE == 4
					if ( 200 <= xhrRequest.status && xhrRequest.status < 400 ) {
		...
		_after_markup_append_process: function() {
			...
			let quick_view 		   = document.querySelector( '#ast-quick-view-modal' ),
				...
				if ( quick_view_box.length > 0 ) {
					...
					if ( variationsForm.length > 0 ) {
						// Only reset the matched variation ID, not the attribute selects.
						// WooCommerce already renders default attribute values in the
						// AJAX HTML — clearing the selects discards those defaults.
			...
			setTimeout(function() {
				...
				// ----------------------------------------------------------------------------
				// TEMPORARY WORKAROUND — CartFlows Variation Swatches compatibility.
				// CartFlows' setSwatchesSelection() runs once on DOMContentLoaded and reads
				// select values to apply cfvsw-selected-swatch to matching swatches.
				// It never re-runs for dynamically loaded content, so we replicate the same
				// logic here, scoped to the Quick View modal.
				// Coupling: .cfvsw-hidden-select, .cfvsw-swatches-option, [data-slug]
				// TODO: Remove once CartFlows exposes a public reinit API or listens to a
				//       standard WooCommerce dynamic-content event.
				// ----------------------------------------------------------------------------
				quick_view_box.find('.cfvsw-hidden-select select').each(function() {
