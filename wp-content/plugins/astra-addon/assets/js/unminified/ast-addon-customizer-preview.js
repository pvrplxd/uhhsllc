	...
	wp.customize( control, function( value ) {
		value.bind( function( value ) {
			...
			if ( value.desktop || value.mobile || value.tablet ) {
				...
				const dynamicStyle = selector + '	{ ' + DeskVal + ' }'
					+ '@media (max-width: ' + tablet_break_point + 'px) {' + selector + '	{ ' + TabletFontVal + ' } }'
					+ '@media (max-width: ' + mobile_break_point + 'px) {' + selector + '	{ ' + MobileVal + ' } }';


				astra_add_dynamic_css( control + '-' + addon, dynamicStyle );

			} else {
				astra_add_dynamic_css( control + '-' + addon, '' );
			}
	...
	wp.customize( control, function( value ) {
		value.bind( function( bg_obj ) {
			...
			if( '' === bg_obj[device] || undefined === bg_obj[device] ){
				astra_add_dynamic_css( control + '-' + device + '-' + addon, '' );
				return;
			}
			...
			if ( 'desktop' == device ) {
				dynamicStyle = selector + '	{ ' + gen_bg_css + ' }';
			}
			if ( 'tablet' == device ) {
				dynamicStyle = '@media (max-width: ' + tablet_break_point + 'px) {' + selector + '	{ ' + gen_bg_css + ' } }';
			}
			if ( 'mobile' == device ) {
				dynamicStyle = '@media (max-width: ' + mobile_break_point + 'px) {' + selector + '	{ ' + gen_bg_css + ' } }';
			}

			astra_add_dynamic_css( control + '-' + device + '-' + addon, dynamicStyle );
		});
	});
}
...
function astra_responsive_background_obj_css( wp_customize, bg_obj, ctrl_name, style, device ) {
	...
	var bg_color	= bg_obj[device]['background-color'];
		...
		if ( 'tablet' == device ) {
			var dynamicStyle = '@media (max-width: ' + tablet_break_point + 'px) {' + style.replace( "{{css}}", gen_bg_css ) + '};';
		}
		if ( 'mobile' == device ) {
			var dynamicStyle = '@media (max-width: ' + mobile_break_point + 'px) {' + style.replace( "{{css}}", gen_bg_css ) + '};';
		}
