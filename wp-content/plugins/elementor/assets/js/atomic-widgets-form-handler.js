...
function registerAtomicFormAlpineData(form) {
  ...
  const alpineId = (0, _utils.getAlpineId)(form);
  if (!alpineId) {
    return;
  }
...
function buildAtomicFormPayload(form) {
  const postId = (0, _utils.getPostId)(form);
...
function getAtomicFormFields(form) {
  const fields = [];
  const allInputs = [...form.querySelectorAll(_utils.ATOMIC_FORM_FIELD_SELECTOR)].filter(input => input.dataset.interactionId);
  const inputGroups = _lodash.default.groupBy(allInputs, input => input.getAttribute('name'));
...
/******/ var __webpack_exports__ = (__webpack_exec__("../modules/atomic-widgets/assets/js/frontend/form-handlers.js"));
/******/ }
]);
