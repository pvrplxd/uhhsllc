...
class GridContainer extends elementorModules.frontend.handlers.Base {
  ...
  static buildCalculationKey(handler) {
    const device = elementor.channels.deviceMode.request('currentMode'),
      ...
      keyParts = [device, gridAutoFlow, rowsSize, rowsUnit, columnsSize, columnsUnit],
      {
  ...
  static getCachedMaxOutlineElementsNumber(handler, childrenLength) {
    ...
    const {
      ...
      } = GridContainer.getDimensionsCache(handler),
      {
        grid_auto_flow: gridAutoFlow
      } = handler.getElementSettings(),
  ...
  updateGridOutline() {
    ...
    if (numberOfItems > 0 && gridOutlineElement?.isConnected && gridOutlineElement.parentElement === this.elements.outlineParentContainer && gridOutlineElement.childElementCount === numberOfItems) {
  ...
  onElementChange(propertyName) {
    ...
    let propsThatTriggerGridLayoutRender = ['grid_rows_grid', 'grid_columns_grid', 'grid_gaps', 'grid_outline', 'container_type', 'boxed_width', 'content_width', 'width', 'height', 'min_height', 'padding', 'grid_auto_flow'];
...
//# sourceMappingURL=container-editor-handlers.b45e73e31bdb50fb2cd1.bundle.js.map